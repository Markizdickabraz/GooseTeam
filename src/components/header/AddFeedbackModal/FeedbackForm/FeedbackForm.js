...
import { Form, RatingText, Label, CommentText, BtnSave, BtnSaveText, BtnCancel, BtnCancelText, BtnWrapper } from './FeedbackForm.styled';
...
const FeedbackForm = ({close, fetchData, setOnModal}) => {
    ...
    await axios.post(`https://goosetrack-backend-2lsp.onrender.com/api/reviews/`, newReview).then(() => {
      setNewComment('');
      setNewRate(0);
      fetchData();
    });
                ...
                <Label>
                  <RatingText>Review</RatingText>
                    <CommentText
                        ...
                        cols="30"
                        rows="10"
                        onChange={event => {setNewComment(event.target.value)}}></CommentText>
                </Label>
                {!isVisible && (
                   <p>
